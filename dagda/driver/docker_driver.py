...
class DockerDriver:
    ...
    def __init__(self, registry_user=None, registry_pass=None, registry_url=None):
        ...
        if registry_user is not None and registry_pass is not None:
            self.auth_creds = {'username': registry_user, 'password': registry_pass}
        try:
        ...
        try:
            #automatically login if there is user and password credential passed in
            if self.auth_creds is not None:
                self.cli.login(username=registry_user, password=registry_pass, registry=registry_url)
        except DockerException:
            DagdaLogger.get_logger().error('Error login, check your username and password')
    ...
    def get_image(self, image_name):
        return self.cli.images(name=image_name)
 
    # return detail info about image, similar to docker inspect
    def get_image_inspect_info(self, image_name):
        return self.cli.inspect_image(image_name)
    
    #get image digest and platform info
    def get_image_info(self, image_name):
        if self.auth_creds:
            return self.cli.inspect_distribution(image_name, auth_config=self.auth_creds)
        else:
            return self.cli.inspect_distribution(image_name)
    ...
    # Docker pull
    def docker_pull(self, image_name, tag='latest', useAuth=True):
        #  pass in auth_config if repository is private
        #https://docker-py.readthedocs.io/en/stable/api.html?highlight=login
        #try with user, password, if failed, try without credential for public images
        
        try:
            if useAuth:
                return self.cli.pull(image_name, tag=tag, auth_config=self.auth_creds)
            else:
                return self.cli.pull(image_name, tag=tag)
        except docker.errors.APIError as ex:
            DagdaLogger.get_logger().error(" pull failed %s trying again"+str(ex))
            #try again with without auth, if with auth failed.
            if useAuth:
                return self.cli.pull(image_name, tag=tag)
            else:
                return self.cli.pull(image_name, tag=tag, auth_config=self.auth_creds)
