...
class MongoDbDriver:
    ...
    def get_docker_image_history(self, image_name, id=None, image_digest=None):
        
        if image_digest:
            cursor = self.db.image_history.find({'image_name': image_name, 'image_digest': image_digest}).sort("timestamp", pymongo.DESCENDING)
        elif not id:
