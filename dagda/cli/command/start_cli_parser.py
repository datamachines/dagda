...
class StartCLIParser:
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--registry_user', type=str)
        self.parser.add_argument('--registry_pass', type=str)
        self.parser.add_argument('--registry_url', type=str)
    ...
    # Gets private docker registry username
    def get_registry_user(self):
        return self.args.registry_user
     # Gets private docker registry pass
    def get_registry_pass(self):
        return self.args.registry_pass
    
    def get_registry_url(self):
        return self.args.registry_url
    # Gets falco rules
    ...
    def verify_args(args):
        if args.server_port and args.server_port not in range(1, 65536):
            ...
            DagdaLogger.get_logger().error('Argument --mongodb_user: this argument should not be empty if you set '
                                           '"--mongodb_pass".')
            return 4
        elif args.registry_user and not args.registry_pass:
            DagdaLogger.get_logger().error('Argument --registry_pass: this argument should not be empty if you set '
                                           '"--registry_user".')
            return 5
        elif args.registry_pass and not args.registry_user:
            DagdaLogger.get_logger().error('Argument --registry_user: this argument should not be empty if you set '
                                           '"--registry_pass".')
            return 6
        elif args.registry_url and not args.registry_url.startswith("http"):
            DagdaLogger.get_logger().error('Argument --registry_url: this argument should starts with http. eg https://index.docker.io/v1/ '
                                           '"--registry_url".')
            return 7
...
Optional Arguments:
  ...
  --mongodb_pass        password for basic authentication with MongoDB. By
                        default, authentication is disabled
  --registry_user       username for basic authentication with private docker registry. By
                        default, assuming registry is public, authentication is disabled
  --registry_pass        password for basic authentication with private docker registry. By
                        default, assuming registry is public, authentication is disabled
  --registery_url       registry url to login on initialization. default https://index.docker.io/v1/
